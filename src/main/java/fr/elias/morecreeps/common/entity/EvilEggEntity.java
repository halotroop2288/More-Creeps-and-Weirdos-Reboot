...
public class EvilEggEntity extends ItemEntity
{
    ...
    public void onUpdate(World world)
    {
        ...
        if (motionY != d1)
        {
            if (rand.nextInt(40) == 0)
            {
                ...
                if(!world.isRemote)
                world.spawnEntityInWorld(creepsentityevilchicken2);
            }
            else
            {
                explode();
    ...
    private void explode(World world)
    {
        if (!exploded)
        {
            exploded = true;
            if(!world.isRemote)
            world.createExplosion(owner, posX, posY, posZ, 2.0F, true);
        }
    }
    ...
    {
    	return new ItemStack(MoreCreepsReboot.evilegg, 1, 0);
    }
}
