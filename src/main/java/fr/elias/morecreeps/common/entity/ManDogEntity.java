...
public class ManDogEntity extends MobEntity
{
        ...
        if (tamed)
        {
            ...
            if (chase)
            {
                ...
                if (Math.abs(frisbeeent.posX - posX) < 1.0D && Math.abs(frisbeeent.posY - posY) < 1.0D && Math.abs(frisbeeent.posZ - posZ) < 1.0D)
                {
                    ...
                    frisbeestack = new ItemStack(MoreCreepsReboot.frisbee, 1, 0);
                    frisbeeent.setDead();
                    chase = false;
                    fetch = true;
                    frisbeehold = true;
                ...
                if (Math.abs(frisbeeent.posX - posX) < 5D && Math.abs(frisbeeent.posZ - posZ) < 5D && frisbeeent.motionX == 0.0D)
                {
                    ...
                    if (attempts > 100)
                    {
                        ...
                        fetch = true;
                        frisbeehold = false;
                        EntityPlayer entityplayer1 = world.getClosestPlayerToEntity(this, 50D);
            ...
            if (fetch && frisbeeent != null)
            {
                if (Math.abs(frisbeeent.posX - posX) < 2D && Math.abs(frisbeeent.posY - posY) < 2D && Math.abs(frisbeeent.posZ - posZ) < 2D)
                {
                    ...
                    if (frisbeehold)
                    {
                        dropItem(MoreCreepsReboot.frisbee, 1);
                    }
    ...
    class AIAttackEntity extends EntityAIBase
    {
		...
		{
			return !(ManDogEntity.this.entityToAttack instanceof EntityPlayer) || !ManDogEntity.this.tamed;
		}
		
		public void updateTask()
		{
			float f = ManDogEntity.this.getDistanceToEntity(getAttackTarget());
			if(f < 256F)
			{
				attackEntity(ManDogEntity.this.getAttackTarget(), f);
				ManDogEntity.this.getLookHelper().setLookPositionWithEntity(ManDogEntity.this.entityToAttack, 10.0F, 10.0F);
				ManDogEntity.this.getNavigator().clearPathEntity();
				ManDogEntity.this.getMoveHelper().setMoveTo(ManDogEntity.this.entityToAttack.posX, ManDogEntity.this.entityToAttack.posY, ManDogEntity.this.entityToAttack.posZ, 0.5D);
			}
			if(f < 1F)
			{
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        if (itemstack != null)
        {
            if (itemstack.getItem() == Items.cooked_porkchop)
            {
                ...
                {
                    smoke();
                    world.playSoundAtEntity(this, "morecreeps:mandogtamed", 1.0F, (rand.nextFloat() - rand.nextFloat()) * 0.2F + 1.0F);
                    tamed = true;
                    this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(45D);
    ...
    private void smoke()
    {
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 10; j++)
            {
                ...
                world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, (posX + (double)(rand.nextFloat() * width * 2.0F)) - (double)width, posY + (double)(rand.nextFloat() * height) + (double)i, (posZ + (double)(rand.nextFloat() * width * 2.0F)) - (double)width, d, d1, d2, new int[0]);
            }
        }
    }
    ...
    public boolean getCanSpawnHere()
    {
        ...
        return i1 != Blocks.sand && i1 != Blocks.cobblestone && i1 != Blocks.log && i1 != Blocks.double_stone_slab && i1 != Blocks.stone_slab && i1 != Blocks.planks && i1 != Blocks.wool && world.getCollidingBoundingBoxes(this, getEntityBoundingBox()).size() == 0 && world.canSeeSky(bp) && rand.nextInt(25) == 0; //&& l > 10;
    }
